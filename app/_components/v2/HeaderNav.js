...
export default function HeaderNav({ statsMap = {} }) {
  ...
  const [notepadText, setNotepadText] = useState('记事本');
  ...
  return (
    <header className={styles.header}>
      ...
      <nav className={styles.nav}>
        ...
        <Link 
          href="https://markdown.520tool.cc" 
          className={styles.notepadEntry} 
          target="_blank" 
          rel="noopener noreferrer"
          onMouseEnter={() => setNotepadText('Markdown')}
          onMouseLeave={() => setNotepadText('记事本')}
        >
          <svg className={styles.notepadIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          <span className={styles.guestbookText}>{notepadText}</span>
        </Link>
